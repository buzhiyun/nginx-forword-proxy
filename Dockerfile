...
RUN sed -i 's#dl-cdn.alpinelinux.org#mirrors.tencent.com#g' /etc/apk/repositories && apk add --no-cache --virtual .build-deps gcc libc-dev make openssl-dev pcre2-dev zlib-dev linux-headers bash alpine-sdk findutils tar patch && tar -zxvf nginx-1.24.0.tar.gz && cd nginx-1.24.0 && \
    ...
    rm -rf /build && apk del --no-network .build-deps && \
    apk add pcre2-dev 
